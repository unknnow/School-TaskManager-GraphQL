...
const Tasks = () => {
  ...
  */
      ...
      return (
        <Col key={task.id}>
          <Card className="h-100">
            <Card.Body>
              <Card.Title>{task.title} - {task.id}</Card.Title>
              <Card.Text>
  ...
  const onSendCommentModal = (e) => {
    createComment({ variables: {content: commentaire, ownerId: "clchjc2s60000uf44f8tzp2fo", parentId: null, targetId: currentTaskId, targetType: "CREATE"} })
      .then((payload) => {
        setCommentaire(null)
        handleClose()
